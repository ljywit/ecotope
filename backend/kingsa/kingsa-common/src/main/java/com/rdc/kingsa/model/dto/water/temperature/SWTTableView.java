...
public class SWTTableView {
	...
	@NotNull(message = "最高水温不能为空")
	@Range(min = -256, max = 99999, message = "最高水温必须在{min}与{max}之间")
	private Double maxWt;

	@ApiModelProperty("最低水温")
	@NotNull(message = "最低水温不能为空")
	@Range(min = -256, max = 99999, message = "最低水温必须在{min}与{max}之间")
	private Double minWt;

	@ApiModelProperty("平均水温")
	@NotNull(message = "平均水温不能为空")
	@Range(min = -256, max = 99999, message = "平均水温必须在{min}与{max}之间")
	private Double aveWt;
	...
	@Length(min = 0, max = 12, message = "校核人不能超过{max}")
	...
	public void setAveWt(Double aveWt) {
		this.aveWt = aveWt;
	}
